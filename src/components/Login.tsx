...
function Login() {
  ...
  return (
    <form onSubmit={handleSubmit} className="form form-padding">
      ...
      <div className= "div">  
      <p>No tienes un usuario? <Link to="/" style={{ color: 'yellow' }} >Regístrate aquí</Link></p> {/* Añade el enlace */}
      </div>
      {mensaje && <p className={mensaje === 'Usuario y/o contraseña inválidos' ? 'error-message' : ''}>{mensaje}</p>}
    </form>
  );
}
